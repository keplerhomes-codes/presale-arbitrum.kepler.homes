...
// Airdrop
export function isClaimed(tokenName, address) {
  const web3 = createCurWeb3()
  return new web3.eth.Contract(Airdrop, getCurAddress()[`Airdrop(${tokenName})`]).methods.isClaimed(address).call()
}

// Airdrop
export function queryClaimers(tokenName) {
  const web3 = createCurWeb3()
  return new web3.eth.Contract(Airdrop, getCurAddress()[`Airdrop(${tokenName})`]).methods.queryClaimers().call()
}
export function claim(
  ...
  return new Promise(async (res, rej) => {
    try{
      ...
      new web3.eth.Contract(Airdrop, getCurAddress()[`Airdrop(${tokenName})`]).methods.claim(
        merkleProof
      )
      .send({from: address})
      .then((result) => {
